...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['name'])) {
        redirectWithAlert("add.php" . ($isEdit ? "?id=$branchId" : ""), "Branch name is required.", "error");
    } else {
        try {
            if ($isEdit) {
            ...
            redirectWithAlert("add.php" . ($isEdit ? "?id=$branchId" : ""), "Error: " . $e->getMessage(), "error");
        }
    }
}
    ...
    <form method="POST">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="manager_id" id="managerIdValue"
                    value="<?php echo htmlspecialchars($branch['manager_id'] ?? ''); ?>">
                <div style="position: relative;">
                    <input type="text" id="managerSearch" class="form-control" placeholder="🔍 Search manager..."
                        autocomplete="off">
                </div>
                <script>
                    document.addEventListener('DOMContentLoaded', function () {
                        const managers = <?php echo json_encode(array_map(function ($u) {
                            return ['id' => $u['id'], 'name' => $u['name']];
                        }, $users)); ?>;

                        // Pre-fill initial value if exists
                        const initialId = "<?php echo $branch['manager_id'] ?? ''; ?>";
                        if (initialId) {
                            const initialManager = managers.find(m => m.id == initialId);
                            if (initialManager) {
                                document.getElementById('managerSearch').value = initialManager.name;
                            }
                        }

                        new SearchableDropdown({
                            inputId: 'managerSearch',
                            hiddenInputId: 'managerIdValue',
                            data: managers,
                            displayField: 'name',
                            valueField: 'id',
