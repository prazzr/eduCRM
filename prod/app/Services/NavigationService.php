...
class NavigationService
{
    ...
    public static function getMenuItems(string $role): array
    {
        ...
        // Tasks (staff only — not teacher/student)
        if (!in_array($role, ['teacher', 'student'])) {
            $items[] = ['label' => 'Tasks', 'url' => $base . 'modules/tasks/list.php', 'icon' => 'check-square', 'section' => 'main'];
        }

        // CRM
        if (in_array($role, ['admin', 'counselor', 'branch_manager', 'accountant'])) {
        ...
        // LMS
        if (in_array($role, ['admin', 'teacher', 'student', 'branch_manager', 'counselor', 'accountant'])) {
            $items[] = ['label' => 'Classes', 'url' => $base . 'modules/lms/classes.php', 'icon' => 'book-open', 'section' => 'lms'];
        }
        if (in_array($role, ['admin', 'teacher', 'branch_manager', 'counselor', 'accountant'])) {
        ...
        // Analytics — Reports: admin, branch_manager, accountant
        if (in_array($role, ['admin', 'branch_manager', 'accountant'])) {
            $items[] = ['label' => 'Reports', 'url' => $base . 'modules/reports/dashboard.php', 'icon' => 'bar-chart-2', 'section' => 'reports'];
        }
        // Analytics — Activity: admin, branch_manager only
        ...
        // Tools — admin and branch_manager only
        if (in_array($role, ['admin', 'branch_manager'])) {
            ...
            $items[] = ['label' => 'Messaging', 'url' => $base . 'modules/messaging/campaigns.php', 'icon' => 'message-circle', 'section' => 'tools'];
