...
function logAction($action, $details = ''): void
{
    ...
    try {
        ...
        // 2. Log to Student Logs (Timeline) if relevant
        // Check if action involves a student ID (extracted from details or context)
        // Common pattern: "Created student ID: 123" or "Updated student ID: 123"
        $studentActions = [
            'student_create',
            'student_update',
            'document_upload',
        ...
        if (
            preg_match('/student ID[:\s]+(\d+)/i', $details, $matches) ||
            strpos($action, 'student_') === 0 ||
            in_array($action, $studentActions)
        ) {

            $student_id = null;
            if (isset($matches[1])) {
                $student_id = (int) $matches[1];
            } elseif ($action === 'document_upload' && preg_match('/Student (\d+)/i', $details, $m)) {
                $student_id = (int) $m[1];
            }

            if ($student_id && $user_id) {
                // Map system actions to readable timeline messages
                $timeline_msg = $details;
                $cat = 'note'; // Use 'note' for enum compatibility

                if ($action == 'student_create') {
                    $timeline_msg = "🎉 Student profile created";
                }
                if ($action == 'student_update') {
                    $timeline_msg = "✏️ Profile updated";
                }
                if ($action == 'document_upload') {
                    // Extract filename from details if available
                    if (preg_match('/uploaded (.+) for/i', $details, $fn)) {
                        $timeline_msg = "📄 Document uploaded: " . $fn[1];
                    } else {
                        $timeline_msg = "📄 Document uploaded";
                    }
                }
                if ($action == 'enrollment') {
                    // Extract class info from details
                    if (preg_match('/in (.+)$/i', $details, $ci)) {
                        $timeline_msg = "📚 Enrolled in class: " . $ci[1];
                    } else {
                        $timeline_msg = "📚 Enrolled in a class";
                    }
                }
                if ($action == 'fee_assigned') {
                    // Extract fee info from details
                    if (preg_match('/- (.+)$/i', $details, $fi)) {
                        $timeline_msg = "💰 Fee assigned: " . $fi[1];
                    } else {
                        $timeline_msg = "💰 Fee assigned";
                    }
                }
                if ($action == 'payment_received') {
                    // Extract payment info from details
                    if (preg_match('/Rs\. ([\d,]+) via (.+)$/i', $details, $pi)) {
                        $timeline_msg = "💵 Payment received: Rs. " . $pi[1] . " (" . $pi[2] . ")";
                    } else {
                        $timeline_msg = "💵 Payment received";
                    }
                }
                ...
                $slog = $pdo->prepare("INSERT INTO student_logs (student_id, author_id, type, message) VALUES (?, ?, ?, ?)");
                $slog->execute([$student_id, $user_id, $cat, $timeline_msg]);
            }
        }
