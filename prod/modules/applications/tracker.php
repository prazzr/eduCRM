...
// Admin/Counselor/Branch Manager only
if (hasRole('student')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_app'])) {
    ...
    // Auto-set branch_id from student's branch (Phase 4 Logic)
    $stmt = $pdo->prepare("SELECT branch_id FROM users WHERE id = ?");
    $stmt->execute([$student_id]);
    $studentBranchId = $stmt->fetchColumn();

    $stmt = $pdo->prepare("INSERT INTO university_applications (student_id, university_name, course_name, country, branch_id) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$student_id, $uni, $course, $country, $studentBranchId]);

    // Log Activity
    $logMsg = "Application started for {$uni} ({$country})";
    if ($course)
        $logMsg .= " - {$course}";
    $pdo->prepare("INSERT INTO student_logs (student_id, author_id, type, message) VALUES (?, ?, 'note', ?)")
...
// Fetch Students (Multi-role support + Branch Filter)
$studentBranchFilter = $branchService->getBranchFilter($_SESSION['user_id'], 'u');
$students = $pdo->query("
...
// Fetch Applications (+ Branch Filter)
$appBranchFilter = $branchService->getBranchFilter($_SESSION['user_id'], 'ua');
$apps = $pdo->query("
    SELECT ua.*, u.name as student_name 
    FROM university_applications ua 
    JOIN users u ON ua.student_id = u.id 
    WHERE 1=1 $appBranchFilter
