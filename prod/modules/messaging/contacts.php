...
// Get branch filter for branch managers
$branchService = new \EduCRM\Services\BranchService($pdo);
$userBranch = $branchService->getUserBranch($_SESSION['user_id']);
$isBranchManager = hasRole('branch_manager') && !hasRole('admin');
$branchId = ($isBranchManager && $userBranch) ? (int) $userBranch['id'] : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'sync_students') {
        // Branch managers only sync students from their branch
        $studentSql = "SELECT id, name, email, phone FROM users WHERE role = 'student'";
        $studentParams = [];
        if ($branchId) {
...
$sql = "SELECT mc.* FROM messaging_contacts mc WHERE 1=1";
$params = [];

// Branch managers see only contacts linked to students in their branch, plus custom contacts
if ($branchId) {
    $sql .= " AND (mc.entity_type != 'student' OR mc.entity_id IN (SELECT id FROM users WHERE branch_id = ?))";
    $params[] = $branchId;
}

if ($search) {
    $sql .= " AND (mc.name LIKE ? OR mc.phone_number LIKE ? OR mc.email LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
