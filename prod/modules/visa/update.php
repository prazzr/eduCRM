    ...
    if ($transitionValid) {
        ...
        if ($data['workflow_id']) {
            ...
            $ins = $pdo->prepare("INSERT INTO visa_workflows 
                (student_id, country_id, stage_id, checklist_json, priority, stage_started_at, expected_completion_date, notes, branch_id) 
                VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?)");
            $ins->execute([$student_id, $country_id, $stage_id, $checklist_json, $priority, $expected_date, $notes, $data['branch_id']]);
        ...
        if ($old_stage_id && $old_stage_id != $stage_id && $data['workflow_id']) {
            try {
                ...
                logAction('notification_sent', "Visa stage update email sent to Student ID: $student_id - Stage: $stageName");
            } catch (Exception $e) {
