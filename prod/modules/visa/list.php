...
// Get pipeline stats for summary cards (Scoped by branch)
$vwBranchFilter = $branchService->getBranchFilter($_SESSION['user_id'], 'vw');
$pipelineStats = $pdo->query("
    SELECT vs.name as stage, COUNT(vw.id) as count
    FROM visa_stages vs
    LEFT JOIN visa_workflows vw ON vw.stage_id = vs.id $vwBranchFilter
