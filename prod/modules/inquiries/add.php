    ...
    if (!csrf_verify()) {
        ...
        if ($name) {
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO inquiries (name, email, phone, country_id, intended_course, education_level_id, source, source_other, assigned_to, status_id, priority_id, branch_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 3, ?)");
                $stmt->execute([$name, $email, $phone, $country_id, $course, $education_level_id, $source, $source_other, $assigned_to, $branch_id]);
