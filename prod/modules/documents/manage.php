...
if ($entityType && $entityId) {
    ...
    // Get all documents (admin view, branch-filtered for branch managers)
    $sql = "SELECT d.*, u.name as uploaded_by_name FROM documents d LEFT JOIN users u ON d.uploaded_by = u.id WHERE 1=1";
    $params = [];

    // Branch managers see only documents uploaded by users in their branch
    if ($branchId) {
        $sql .= " AND u.branch_id = ?";
        $params[] = $branchId;
    }
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                <div class="p-6">
                    ...
                    <div class="flex gap-2">
                        ...
                        <?php if (hasRole('admin') || hasRole('branch_manager') || $doc['uploaded_by'] == $_SESSION['user_id']): ?>
