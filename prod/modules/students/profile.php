...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_log']) && !hasRole('student')) {
    ...
    header("Location: profile.php?id=" . $student_id . "&tab=logs");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_fee']) && (hasRole('admin') || hasRole('branch_manager'))) {
    try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_payment']) && (hasRole('admin') || hasRole('branch_manager'))) {
    try {
        $fee_id = $_POST['fee_id'];
        $amt = (float) $_POST['amount'];
        $method = $_POST['method'];
...
if (isset($_GET['delete_fee']) && (hasRole('admin') || hasRole('branch_manager'))) {
            ...
            <tbody>
                ...
                <?php foreach ($all_fees as $f): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($f['status'] == 'unpaid' && (hasRole('admin') || hasRole('branch_manager'))): ?>
...
<?php elseif ($tab === 'logs'): ?>
    ...
    <?php endif; ?>
    <h3 style="margin-top: 25px; margin-bottom: 10px;">Activity Timeline</h3>
    <?php if (count($timelines) > 0): ?>
        <table
            style="width:100%; border-collapse: collapse; font-size: 13px; background: #fff; border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden;">
            <thead>
                <tr style="background: #f8fafc; border-bottom: 1px solid #e2e8f0; text-align: left; color: #64748b;">
                    <th style="padding: 6px 10px; width: 140px;">Date</th>
                    <th style="padding: 6px 10px; width: 50px; text-align: center;">Type</th>
                    <th style="padding: 6px 10px;">Activity Logs</th>
                    <th style="padding: 6px 10px; width: 150px;">Author</th>
                    <?php if (!hasRole('student')): ?>
                        <th style="padding: 6px 10px; width: 50px; text-align: right;">Action</th>
                    <?php endif; ?>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($timelines as $log): ?>
                    <?php
                    $icons = ['call' => 'phone', 'email' => 'mail', 'meeting' => 'users', 'note' => 'file-text'];
                    $colors = ['call' => '#2563eb', 'email' => '#d97706', 'meeting' => '#9333ea', 'note' => '#64748b'];
                    $iconName = $icons[$log['type']] ?? 'circle';
                    $color = $colors[$log['type']] ?? '#64748b';
                    ?>
                    <tr style="border-bottom: 1px solid #e2e8f0;">
                        ...
                        <td style="padding: 6px 10px; text-align: center;">
                            <span style="color: <?php echo $color; ?>;" title="<?php echo ucfirst($log['type']); ?>">
                                <?php echo \EduCRM\Services\NavigationService::getIcon($iconName, 18); ?>
                            </span>
                        </td>
                        ...
                        <?php if (!hasRole('student')): ?>
                            <td style="padding: 6px 10px; text-align: right;">
                                <a href="?id=<?php echo $student_id; ?>&tab=logs&delete_log=<?php echo $log['id']; ?>"
                                    ...
                                    style="color: #ef4444; text-decoration: none; font-weight: bold; font-size: 16px;">×</a>
                            </td>
                        <?php endif; ?>
                    </tr>
                <?php endforeach; ?>
