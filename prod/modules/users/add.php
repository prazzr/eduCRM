...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($selected_role_ids)) {
        ...
        if (hasRole('admin')) {
            // Admins have no role restrictions
        } elseif (hasRole('branch_manager')) {
            // Branch Managers cannot create Admins
            $admin_role = null;
            foreach ($all_roles as $r) { if ($r['name'] === 'admin') { $admin_role = $r; break; } }

            if ($admin_role) {
                foreach ($selected_role_ids as $rid) {
                    if ($rid == $admin_role['id']) {
                        redirectWithAlert("add.php", "Unauthorized: Branch Managers cannot create Administrator accounts.", 'error');
                    }
                }
            }
        } elseif (hasRole('counselor')) {
            // Counselors can only create Students
            $student_role = null;
            foreach ($all_roles as $r) { if ($r['name'] === 'student') { $student_role = $r; break; } }
            
            if ($student_role) {
                foreach ($selected_role_ids as $rid) {
                    if ($rid != $student_role['id']) {
        ...
        if ($check->rowCount() > 0) {
            ...
            try {
                ...
                } elseif (hasRole('admin') && !empty($_POST['branch_id'])) {
                    $target_branch_id = $_POST['branch_id'];
                }
