...
class DocumentService
{
    ...
    public function uploadDocument($file, $entityType, $entityId, $category = null, $description = null)
    {
        ...
        if ($result) {
            ...
            // Auto-sync with student_documents checklist if this is a student upload
            if ($entityType === 'student' && $category) {
                $this->syncChecklistStatus($entityId, $category, $filepath, $file['name']);
            }
    ...
    /**
     * Sync uploaded document with student_documents checklist
     * Auto-sets status to 'uploaded' when a matching document type is uploaded
     */
    private function syncChecklistStatus($studentId, $category, $filePath, $originalFilename)
    {
        // Find matching document type by code
        $stmt = $this->pdo->prepare("SELECT id FROM document_types WHERE code = ?");
        $stmt->execute([$category]);
        $docType = $stmt->fetch(\PDO::FETCH_ASSOC);

        if ($docType) {
            // Upsert into student_documents with 'uploaded' status
            $upsert = $this->pdo->prepare("
                INSERT INTO student_documents 
                (student_id, document_type_id, status, file_path, original_filename, uploaded_by)
                VALUES (?, ?, 'uploaded', ?, ?, ?)
                ON DUPLICATE KEY UPDATE 
                    status = IF(status IN ('pending', 'rejected'), 'uploaded', status),
