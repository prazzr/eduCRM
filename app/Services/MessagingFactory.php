...
require_once 'gateways/NtfyGateway.php';
...
class MessagingFactory
{
    ...
    public static function create($gatewayId = null, $type = 'sms')
    {
        ...
        switch ($gateway['provider']) {
            case 'twilio':
                return new \EduCRM\Services\gateways\TwilioGateway(self::$pdo, $gateway);

            case 'smpp':
                return new \EduCRM\Services\gateways\SMPPGateway(self::$pdo, $gateway);

            case 'gammu':
                return new \EduCRM\Services\gateways\GammuGateway(self::$pdo, $gateway);

            // WhatsApp gateways (Phase 3C)
            case 'twilio_whatsapp':
            case 'whatsapp_business':
            case '360dialog':
                return new \EduCRM\Services\gateways\WhatsAppGateway(self::$pdo, $gateway);

            // Viber gateway (Phase 3D)
            case 'viber_bot':
                return new \EduCRM\Services\gateways\ViberGateway(self::$pdo, $gateway);

            // ntfy gateway (Self-Hosted Push)
            case 'ntfy':
                return new \EduCRM\Services\gateways\NtfyGateway(self::$pdo, $gateway);
